...
var records = mustValidate([]Record{
	...
	{Agent: "codex-desktop", Version: "0.154.0", Platform: "darwin", Mode: domain.ModeLaunch, Surface: domain.SurfaceModelPrimary, Protocol: domain.ProtocolOpenAIResponses, Auth: domain.AuthPassthrough, Coverage: domain.CoverageProtected, Verification: VerificationLaunchSmoke, Notes: "bundled Codex app-server accepted isolated CODEX_HOME provider configuration on macOS; main model route only; live provider request and App tool descendant egress not verified"},
