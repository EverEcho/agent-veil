...
let lastTestResults=[];
for(const id of ['message','policy-result','rule-install-result','model-install-result','test-result','diagnostic-result']){const node=document.querySelector('#'+id);node.setAttribute('role','status');node.setAttribute('aria-live','polite')}
...
function renderRisks(risks){return(risks||[]).map(risk=>'<div class="route"><span class="status '+e(risk.severity||'high')+'">'+e(risk.severity||'high')+'</span> · '+e(risk.code||'UNKNOWN_RISK')+'<br><span class="muted">Source '+e(risk.surface_id||'unknown Surface')+' · Impact: '+e(risk.message||'Protection cannot be proven')+' · Action: '+e(riskAction(risk.code))+'</span></div>').join('')}
...
function renderTrends(events){const now=new Date(),today=localDay(now),todayEvents=(events||[]).filter(event=>localDay(event.timestamp)===today),actions={allow:0,redact:0,block:0,ask:0};let findings=0,errors=0;for(const event of todayEvents){if(Object.hasOwn(actions,event.action))actions[event.action]++;findings+=Number(event.finding_count||0);if(event.error_code)errors++}const days=[];for(let offset=6;offset>=0;offset--){const day=new Date(now);day.setHours(12,0,0,0);day.setDate(day.getDate()-offset);const key=localDay(day),matching=(events||[]).filter(event=>localDay(event.timestamp)===key);days.push('<p>'+e(key)+' · '+matching.length+' decisions · '+matching.reduce((total,event)=>total+Number(event.finding_count||0),0)+' findings · '+matching.filter(event=>event.action==='block'||event.error_code).length+' blocked/errors</p>')}return'<section class="card"><h2>Today</h2><p>'+todayEvents.length+' decisions · '+findings+' findings · '+errors+' errors</p><p class="muted">Allow '+actions.allow+' · Redact '+actions.redact+' · Block '+actions.block+' · Ask '+actions.ask+'</p></section><section class="card"><h2>Recent metadata trend</h2>'+days.join('')+'</section>'}
function renderRoutes(rows){return rows.map(entry=>{const routes=Object.fromEntries((entry.plan.routes||[]).map(route=>[route.surface_id,route])),coverage=Object.fromEntries((entry.plan.coverage||[]).map(item=>[item.surface_id,item]));return'<section class="card"><h2>'+e(entry.manifest.agent.kind)+'</h2>'+(entry.manifest.surfaces||[]).map(surface=>{const route=routes[surface.id],status=coverage[surface.id]||{status:'unprotected',reason:'No coverage result'},upstream=route&&route.upstream||surface.upstream||{},auth=route&&route.auth||surface.auth||{},network=route&&route.network||surface.network||{},policy=route&&route.policy_id||'none';return'<div class="route"><span class="status '+e(status.status)+'">'+e(status.status)+'</span> · '+e(surface.name)+'<br><span class="muted">'+e(surface.type)+' → '+e(route&&route.protocol||surface.protocol||'unknown')+' → policy '+e(policy)+' → '+e(endpointText(upstream))+' → auth '+e(auth.type||'unknown')+' → network '+e(network.type||'unknown')+'</span><br><span class="muted">'+e(status.reason||'')+'</span></div>'}).join('')+'</section>'}).join('')}
...
function policyScope(scope){if(!scope)return'default policy';return Object.entries(scope).filter(entry=>entry[1]).map(entry=>entry[0]+'='+entry[1]).join(' · ')||'all findings'}
function redactionPreview(finding){const location=finding.location||{},length=Math.max(0,Number(location.end||0)-Number(location.start||0));return'[REDACTED:'+String(finding.category||'sensitive')+'] replaces '+length+' bytes'}
function ruleExplanation(finding){const category=String(finding.category||'');if(category.startsWith('secret.'))return'Deterministic credential or secret signature';if(category.startsWith('pii.'))return'PII format, validator, or local semantic classification';return'Verified local rule-pack classification'}
async function testRules(){const input=document.querySelector('#test-input'),result=document.querySelector('#test-result'),output=document.querySelector('#test-findings'),scope={agent_id:document.querySelector('#test-agent').value,provider:document.querySelector('#test-provider').value,surface_id:document.querySelector('#test-surface').value,workspace:document.querySelector('#test-workspace').value};for(const key of Object.keys(scope))if(!scope[key])delete scope[key];try{const response=await fetch('/v1/detect',{method:'POST',headers:{Authorization:'Bearer '+token(),'Content-Type':'application/json'},body:JSON.stringify({text:input.value,scope})});input.value='';if(!response.ok)throw Error('scan failed');const payload=await response.json();lastTestResults=payload.results||[];result.className='active';result.textContent=lastTestResults.length+' findings with policy decisions; input cleared';output.innerHTML=lastTestResults.map((item,index)=>{const finding=item.finding||{},decision=item.decision||{},confidence=Math.max(0,Math.min(100,Math.round(Number(finding.confidence||0)*100)));return'<div class="route"><span class="status '+e(decision.action)+'">'+e(decision.action||'unknown')+'</span> · <span class="status '+e(finding.severity)+'">'+e(finding.severity)+'</span> · '+e(finding.category)+' · bytes '+Number(finding.location&&finding.location.start)+'–'+Number(finding.location&&finding.location.end)+'<br><span class="muted">Rule '+e(finding.rule_id||'unknown')+' · '+e(finding.detector||'unknown')+' · '+confidence+'% confidence · detector suggests '+e(finding.suggested_action||'unknown')+'<br>'+e(ruleExplanation(finding))+'<br>'+e(decision.reason||'Policy decision unavailable')+' · '+e(policyScope(decision.matched_scope))+'</span><div class="controls"><button class="false-positive" data-index="'+index+'">Mark false positive</button></div></div>'}).join('')||'<p class="muted">No deterministic findings</p>'}catch(err){input.value='';lastTestResults=[];output.textContent='';result.className='error';result.textContent='Scan or policy preview rejected; input cleared'}}
...
async function inspectAgent(name){const output=document.querySelector('#inspection');try{const response=await fetch('/v1/discovery/'+encodeURIComponent(name),{headers:{Authorization:'Bearer '+token()}});if(!response.ok)throw Error('inspection failed');const x=await response.json(),coverage=Object.fromEntries(x.protection_plan.coverage.map(item=>[item.surface_id,item]));output.innerHTML='<section class="card"><h2>'+e(x.manifest.agent.kind)+' <span class="muted">'+e(x.manifest.agent.version)+'</span></h2><p>Protected '+x.protection_plan.summary.protected+' · Local '+x.protection_plan.summary.local+' · Partial '+x.protection_plan.summary.partial+' · Observed '+x.protection_plan.summary.observed+' · Unprotected '+x.protection_plan.summary.unprotected+'</p>'+x.manifest.surfaces.map(surface=>{const status=coverage[surface.id]||{status:'unprotected',reason:'No coverage result'};return '<p><span class="status '+e(status.status)+'">'+e(status.status)+'</span> · '+e(surface.name)+' · '+e(surface.protocol)+'<br><span class="muted">'+e(status.reason)+'</span></p>'}).join('')+(x.protection_plan.risks.length?'<h3>Risks and actions</h3>'+renderRisks(x.protection_plan.risks):'')+'</section>'}catch(err){output.innerHTML='<p class="error">Inspection unavailable; no control changes were made.</p>'}}
async function load(){const m=document.querySelector('#message');try{const headers={Authorization:'Bearer '+token()},responses=await Promise.all([fetch('/v1/agents',{headers}),fetch('/v1/approvals',{headers}),fetch('/v1/audit',{headers}),fetch('/v1/call-tree',{headers}),fetch('/v1/discovery',{headers})]);if(responses.some(response=>!response.ok))throw Error('unauthorized');const rows=await responses[0].json(),asks=await responses[1].json(),events=await responses[2].json(),calls=await responses[3].json(),detections=await responses[4].json(),sessionCount=Object.values(calls).reduce((total,nodes)=>total+nodes.length,0);m.textContent=detections.length+' installed · '+rows.length+' registered · '+sessionCount+' active sessions · '+events.length+' retained decisions';document.querySelector('#approvals').innerHTML=asks.map(x=>'<section class="card"><div class="status partial">Decision required</div><h2>'+e(x.finding.category)+'</h2><p>'+e(x.finding.severity)+' · '+e(x.finding.location.path)+'</p><p class="muted">Safe preview: '+e(redactionPreview(x.finding))+'</p><button class="approval-decision" data-id="'+e(x.id)+'" data-action="redact">Redact once</button> <button class="approval-decision" data-id="'+e(x.id)+'" data-action="allow">Allow once</button> <button class="approval-decision" data-id="'+e(x.id)+'" data-action="block">Cancel and block</button></section>').join('');document.querySelector('#discovered').innerHTML=detections.map(x=>'<section class="card"><div class="status '+e(x.status==='verified'?'active':'observed')+'">'+e(x.status)+'</div><h2>'+e(x.agent)+'</h2><p class="muted">'+e(x.version||'unknown version')+'</p><button class="inspect-agent" data-agent="'+e(x.agent)+'">Inspect surfaces</button></section>').join('')||'<p class="muted">No supported agents found</p>';document.querySelector('#agents').innerHTML=rows.map(x=>'<section class="card"><div class="status '+e(x.state)+'">'+e(x.state)+'</div><h2>'+e(x.manifest.agent.kind)+'</h2><p class="muted">'+e(x.manifest.agent.version||'unknown version')+'</p><p>Protected '+x.plan.summary.protected+' · Local '+x.plan.summary.local+' · Partial '+x.plan.summary.partial+' · Observed '+x.plan.summary.observed+' · Unprotected '+x.plan.summary.unprotected+'</p>'+renderRisks(x.plan.risks)+'</section>').join('');document.querySelector('#routes').innerHTML=renderRoutes(rows)||'<p class="muted">No registered routes</p>';document.querySelector('#calls').innerHTML=renderCalls(calls,'',0)||'<p class="muted">No active sessions</p>';document.querySelector('#trends').innerHTML=renderTrends(events);document.querySelector('#audit').innerHTML=events.slice(-20).reverse().map(x=>'<section class="card"><div class="status '+e(x.action)+'">'+e(x.action)+'</div><p>'+e(x.agent_id||'unknown')+' · '+e(x.surface_id||'unknown')+'</p><p class="muted">'+e(x.protocol||'unknown')+' · '+Number(x.finding_count||0)+' findings · '+Number(x.latency_ms||0)+' ms'+(x.error_code?' · '+e(x.error_code):'')+'</p></section>').join('')}catch(err){m.textContent='Unable to load protected status';for(const id of ['discovered','agents','routes','calls','trends','audit'])document.querySelector('#'+id).textContent=''}}
document.querySelector('#load').onclick=()=>{load();loadPolicy().catch(()=>{document.querySelector('#policy-result').textContent='Unable to load policy'});loadRulePacks().catch(()=>{document.querySelector('#rule-packs').innerHTML='<p class="error">Rule inventory unavailable.</p>'});loadModels().catch(()=>{document.querySelector('#models').innerHTML='<p class="error">Model inventory unavailable.</p>'})};document.querySelector('#save-policy').onclick=savePolicy;document.querySelector('#install-rule').onclick=installRulePack;document.querySelector('#install-model').onclick=installModel;document.querySelector('#test-rules').onclick=testRules;document.querySelector('#download-diagnostics').onclick=downloadDiagnostics;setInterval(()=>{if(token())load()},1000)
