...
localeChoice.onchange=()=>{dashboardLocale=localeChoice.value;try{localStorage.setItem('agentveil.locale',dashboardLocale)}catch(err){}translateTree(document)};
let lastTestResults=[];
