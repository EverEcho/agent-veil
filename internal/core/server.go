...
:root{color-scheme:dark;font-family:Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#080b12;color:#edf2fa}*{box-sizing:border-box}body{max-width:1240px;margin:0 auto;padding:48px 32px 80px;background:radial-gradient(circle at 12% 0,#1b2440 0,transparent 34%),radial-gradient(circle at 90% 8%,#172f38 0,transparent 28%);min-height:100vh}main{position:relative}h1{font-size:42px;line-height:1;margin:0 0 10px;letter-spacing:-.055em;background:linear-gradient(115deg,#fff 30%,#9aa8ff);-webkit-background-clip:text;color:transparent}h2{margin:38px 0 14px;font-size:17px;letter-spacing:-.02em;color:#dfe6f2}.muted{color:#8e9bb0;line-height:1.55}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:14px}.card{background:linear-gradient(145deg,#151c2aeb,#101620eb);border:1px solid #283349;border-radius:18px;padding:19px;box-shadow:0 12px 38px #0003;transition:border-color .2s,transform .2s}.card:hover{border-color:#3b4965;transform:translateY(-1px)}.route{padding:12px 0;border-top:1px solid #273044}.call{margin:8px 0}.depth-0{margin-left:0}.depth-1{margin-left:24px}.depth-2{margin-left:48px}.depth-3{margin-left:72px}.depth-4{margin-left:96px}.depth-5{margin-left:120px}.depth-6{margin-left:144px}.depth-7{margin-left:168px}.depth-8{margin-left:192px}.status{display:inline-flex;font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.active,.protected,.local,.low,.allow,.redact{color:#57dca0}.blocked,.unprotected,.error,.critical,.high,.block{color:#ff707c}.partial,.observed,.medium,.ask{color:#f7c65c}button,input,textarea,select{background:#161e2c;color:inherit;border:1px solid #344159;border-radius:11px;padding:11px 13px;font:inherit}input,select{min-height:42px}button{cursor:pointer;font-weight:700;background:linear-gradient(145deg,#28344a,#202a3c);box-shadow:0 5px 15px #0003}button:hover{border-color:#6979a3;background:#303d55}button:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible,a:focus-visible{outline:3px solid #7b8dff;outline-offset:3px}textarea{width:100%;min-height:160px;font:13px ui-monospace,SFMono-Regular,monospace;resize:vertical;line-height:1.55}.controls{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:12px}.scope-inputs{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin:12px 0}.scope-inputs input{min-width:0}.locale-picker{display:flex;gap:8px;align-items:center;float:right}.skip{position:absolute;left:12px;top:-80px;background:#e8edf5;color:#0b0e14;padding:10px;z-index:10}.skip:focus{top:12px}@media(max-width:700px){body{padding:32px 18px 64px}h1{font-size:36px}.locale-picker{float:none;margin-bottom:20px}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}</style></head><body>
...
Object.assign(translations['zh-CN'],{'Revoke session':'撤销会话','Session revocation failed':'会话撤销失败','Stop protection':'停止保护','Protection stop failed':'停止保护失败'});
...
Object.assign(messages.en,{'rule.verified':'verified {version}','rule.builtin':'built-in','confirm.rule':'Remove inactive rule pack {version}?','confirm.model':'Remove inactive semantic model {version}?','confirm.session':'Revoke Session {session}? Protected requests using it will stop.','confirm.agent':'Stop protection for {agent} generation {generation}? Its routes will fail closed.'});
Object.assign(messages['zh-CN'],{'rule.verified':'已验证 {version}','rule.builtin':'内置规则','confirm.rule':'移除未启用的规则包 {version}？','confirm.model':'移除未启用的语义模型 {version}？','confirm.session':'撤销会话 {session}？使用该会话的受保护请求将停止。','confirm.agent':'停止 {agent} 第 {generation} 代保护？其路由将安全阻断。'});
...
async function revokeSession(id){if(!window.confirm(message('confirm.session',{session:id})))return;const response=await fetch('/v1/sessions/'+encodeURIComponent(id),{method:'DELETE',headers:{Authorization:'Bearer '+token()}});if(!response.ok)throw Error('session revocation failed');await load()}
async function removeAgent(id,generation){generation=Number(generation);if(!Number.isSafeInteger(generation)||generation<1)throw Error('invalid generation');if(!window.confirm(message('confirm.agent',{agent:id,generation})))return;const response=await fetch('/v1/agents/'+encodeURIComponent(id)+'?generation='+encodeURIComponent(String(generation)),{method:'DELETE',headers:{Authorization:'Bearer '+token()}});if(!response.ok)throw Error('protection stop failed');await load()}
...
function refreshConfiguration(){loadPolicy().catch(()=>{document.querySelector('#policy-result').textContent='Unable to load policy'});loadRulePacks().catch(()=>{document.querySelector('#rule-packs').innerHTML='<p class="error">Rule inventory unavailable.</p>'});loadModels().catch(()=>{document.querySelector('#models').innerHTML='<p class="error">Model inventory unavailable.</p>'})}
document.querySelector('#load').onclick=()=>{load();loadHealth();refreshConfiguration()};document.querySelector('#save-policy').onclick=savePolicy;document.querySelector('#install-rule').onclick=installRulePack;document.querySelector('#install-model').onclick=installModel;document.querySelector('#test-rules').onclick=testRules;document.querySelector('#download-diagnostics').onclick=downloadDiagnostics;setInterval(()=>{if(token()){load();loadHealth()}},1000);setInterval(()=>{if(token())refreshConfiguration()},5000)
...
document.querySelector('#calls').onclick=event=>{const button=event.target.closest('.revoke-session');if(button)revokeSession(button.dataset.session).catch(()=>{document.querySelector('#message').textContent=localizedText('Session revocation failed')})}
document.querySelector('#routes').onclick=event=>{const button=event.target.closest('.remove-agent');if(button)removeAgent(button.dataset.agent,button.dataset.generation).catch(()=>{document.querySelector('#message').textContent=localizedText('Protection stop failed')})}
