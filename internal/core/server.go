...
async function load(){const m=document.querySelector('#message');try{const headers={Authorization:'Bearer '+token()},responses=await Promise.all([fetch('/v1/agents',{headers}),fetch('/v1/approvals',{headers}),fetch('/v1/audit',{headers}),fetch('/v1/call-tree',{headers}),fetch('/v1/discovery',{headers})]);if(responses.some(response=>!response.ok))throw Error('unauthorized');const rows=await responses[0].json(),asks=await responses[1].json(),events=await responses[2].json(),calls=await responses[3].json(),detections=await responses[4].json(),sessionCount=Object.values(calls).reduce((total,nodes)=>total+nodes.length,0);m.textContent=detections.length+' installed · '+rows.length+' registered · '+sessionCount+' active sessions · '+events.length+' retained decisions';document.querySelector('#approvals').innerHTML=asks.map(x=>'<section class="card"><div class="status partial">Decision required</div><h2>'+e(x.finding.category)+'</h2><p>'+e(x.finding.severity)+' · '+e(x.finding.location.path)+'</p><button class="approval-decision" data-id="'+e(x.id)+'" data-action="redact">Redact once</button> <button class="approval-decision" data-id="'+e(x.id)+'" data-action="allow">Allow once</button> <button class="approval-decision" data-id="'+e(x.id)+'" data-action="block">Block</button></section>').join('');document.querySelector('#discovered').innerHTML=detections.map(x=>'<section class="card"><div class="status '+e(x.status==='verified'?'active':'observed')+'">'+e(x.status)+'</div><h2>'+e(x.agent)+'</h2><p class="muted">'+e(x.version||'unknown version')+'</p><button class="inspect-agent" data-agent="'+e(x.agent)+'">Inspect surfaces</button></section>').join('')||'<p class="muted">No supported agents found</p>';document.querySelector('#agents').innerHTML=rows.map(x=>'<section class="card"><div class="status '+e(x.state)+'">'+e(x.state)+'</div><h2>'+e(x.manifest.agent.kind)+'</h2><p class="muted">'+e(x.manifest.agent.version||'unknown version')+'</p><p>Protected '+x.plan.summary.protected+' · Local '+x.plan.summary.local+' · Partial '+x.plan.summary.partial+' · Observed '+x.plan.summary.observed+' · Unprotected '+x.plan.summary.unprotected+'</p>'+renderRisks(x.plan.risks)+'</section>').join('');document.querySelector('#routes').innerHTML=renderRoutes(rows)||'<p class="muted">No registered routes</p>';document.querySelector('#calls').innerHTML=renderCalls(calls,'',0)||'<p class="muted">No active sessions</p>';document.querySelector('#audit').innerHTML=events.slice(-20).reverse().map(x=>'<section class="card"><div class="status '+e(x.action)+'">'+e(x.action)+'</div><p>'+e(x.agent_id||'unknown')+' · '+e(x.surface_id||'unknown')+'</p><p class="muted">'+e(x.protocol||'unknown')+' · '+Number(x.finding_count||0)+' findings · '+Number(x.latency_ms||0)+' ms'+(x.error_code?' · '+e(x.error_code):'')+'</p></section>').join('')}catch(err){m.textContent='Unable to load protected status';for(const id of ['discovered','agents','routes','calls','audit'])document.querySelector('#'+id).textContent=''}}
