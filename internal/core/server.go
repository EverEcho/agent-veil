...
func (s *Server) testDetection(w http.ResponseWriter, r *http.Request) {
	...
	for index := range matches {
		...
		results[index] = detectionTestResult{Finding: matches[index].Finding, Decision: preview}
	}
...
async function testRules(){const input=document.querySelector('#test-input'),result=document.querySelector('#test-result'),output=document.querySelector('#test-findings'),scope={agent_id:document.querySelector('#test-agent').value,provider:document.querySelector('#test-provider').value,surface_id:document.querySelector('#test-surface').value,workspace:document.querySelector('#test-workspace').value};for(const key of Object.keys(scope))if(!scope[key])delete scope[key];try{const response=await fetch('/v1/detect',{method:'POST',headers:{Authorization:'Bearer '+token(),'Content-Type':'application/json'},body:JSON.stringify({text:input.value,scope})});input.value='';if(!response.ok)throw Error('scan failed');const payload=await response.json(),results=payload.results||[];result.className='active';result.textContent=results.length+' findings with policy decisions; input cleared';output.innerHTML=results.map(item=>{const finding=item.finding||{},decision=item.decision||{};return'<div class="route"><span class="status '+e(decision.action)+'">'+e(decision.action||'unknown')+'</span> · <span class="status '+e(finding.severity)+'">'+e(finding.severity)+'</span> · '+e(finding.category)+' · bytes '+Number(finding.location&&finding.location.start)+'–'+Number(finding.location&&finding.location.end)+'<br><span class="muted">'+e(decision.reason||'Policy decision unavailable')+' · '+e(policyScope(decision.matched_scope))+'</span></div>'}).join('')||'<p class="muted">No deterministic findings</p>'}catch(err){input.value='';output.textContent='';result.className='error';result.textContent='Scan or policy preview rejected; input cleared'}}
