...
async function loadHealth(){try{const response=await fetch('/v1/health',{headers:{Authorization:'Bearer '+token()}});if(!response.ok)throw Error('health unavailable');const health=await response.json(),node=document.querySelector('#core-health'),failures=Number(health.audit_failures||0);node.className='status '+(health.status==='ok'?'active':'error');node.textContent='Core '+String(health.status||'unknown')+' · API '+String(health.api_version||'unknown')+' · audit '+String(health.audit||'unknown')+(failures?' ('+failures+' failures)':'')+' · semantic '+String(health.semantic||'unknown')}catch(err){const node=document.querySelector('#core-health');node.className='status error';node.textContent='Core health unavailable'}}
