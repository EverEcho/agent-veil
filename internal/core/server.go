...
Object.assign(translations['zh-CN'],{'Revoke session':'撤销会话','Session revocation failed':'会话撤销失败'});
...
Object.assign(messages.en,{'rule.verified':'verified {version}','rule.builtin':'built-in','confirm.rule':'Remove inactive rule pack {version}?','confirm.model':'Remove inactive semantic model {version}?','confirm.session':'Revoke Session {session}? Protected requests using it will stop.'});
Object.assign(messages['zh-CN'],{'rule.verified':'已验证 {version}','rule.builtin':'内置规则','confirm.rule':'移除未启用的规则包 {version}？','confirm.model':'移除未启用的语义模型 {version}？','confirm.session':'撤销会话 {session}？使用该会话的受保护请求将停止。'});
