...
document.querySelector('#load').onclick=()=>{load();loadPolicy().catch(()=>{document.querySelector('#policy-result').textContent='Unable to load policy'});loadRulePacks().catch(()=>{document.querySelector('#rule-packs').innerHTML='<p class="error">Rule inventory unavailable.</p>'})};document.querySelector('#save-policy').onclick=savePolicy;document.querySelector('#test-rules').onclick=testRules;setInterval(()=>{if(token())load()},1000)
...
document.querySelector('#rule-packs').onclick=event=>{const activate=event.target.closest('.activate-rule');if(activate)activateRulePack(activate.dataset.version).catch(()=>{document.querySelector('#rule-packs').innerHTML='<p class="error">Rule activation failed.</p>'});if(event.target.closest('#use-built-in'))deactivateRulePack().catch(()=>{document.querySelector('#rule-packs').innerHTML='<p class="error">Rule deactivation failed.</p>'})}
</script></body></html>`
