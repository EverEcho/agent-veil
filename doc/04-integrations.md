...
- Codex Desktop 作为独立 Integration 识别其内置 `codex app-server` 版本；macOS 使用 AgentVeil 私有的会话级 `CODEX_HOME` 启动官方 App，仅对会话和 SQLite/WAL 等需要规范单一路径的持久状态使用受控符号链接；可成为沙箱可写根的产物、缓存和工具目录使用真实隔离目录，退出后清理临时认证、配置与运行时状态并保留会话路径和用户产物，不修改 `~/.codex`；
