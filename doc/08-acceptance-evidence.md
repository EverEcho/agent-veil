...
| 3 | 部分通过 | `TestProtocolFixturesExtractOnlyBusinessContentAndRoundTrip`、`TestNonStreamingResponseProtocolMatrixRestoresPlaceholders`、`TestSSEProtocolMatrixRestoresFragmentedPlaceholders`、`TestEndToEndProtocolMatrixOnlySendsRedactedContentToProvider` 覆盖 OpenAI Chat/Responses、Anthropic、Gemini、MCP Streamable HTTP；`TestMCPRoutesUseExactConfiguredUpstreamPath` 固定任意配置端点；`TestLegacyMCPSSEDualEndpointRoundTripProtectsProviderBoundary` 证明 legacy SSE 的 endpoint 改写、GET/POST 共享 Vault、能力绑定和 Provider 边界，`TestCoreKeepsLegacyMCPSSEStateAcrossPerRequestHandlers` 以每类单并发槽证明 Core 生命周期与流/请求分流，其他 manager 测试覆盖有界状态、排空和撤销 | 仍需以 Compatibility Matrix 中的真实 Agent/Provider 版本完成端到端实机验证；Browser、OAuth、文件上传、WebSocket 与需重签名的云协议仍未闭环 |
