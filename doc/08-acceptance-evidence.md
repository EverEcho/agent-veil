...
| 9 | 部分通过 | Planner 禁止将不完整能力标成 Protected；未知版本被降级为风险 Manifest；`TestInspectionPreviewReturnsManifestAndTruthfulCoverage` 证明 Core 预览保留 Observed 与必需 Unprotected 缺口且不创建 Route；Compatibility Matrix 是唯一验证事实源；Dashboard/CLI 从同一 Core API 读取 Agent、Session、Call Tree 与覆盖率 | 当前 Dashboard 是 Core 内嵌本地页面，不是具备托盘/生命周期的正式桌面应用 |
