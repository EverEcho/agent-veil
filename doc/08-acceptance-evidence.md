...
| 3 | 部分通过 | `TestProtocolFixturesExtractOnlyBusinessContentAndRoundTrip`、`TestNonStreamingResponseProtocolMatrixRestoresPlaceholders`、`TestSSEProtocolMatrixRestoresFragmentedPlaceholders`、`TestEndToEndProtocolMatrixOnlySendsRedactedContentToProvider` 覆盖 OpenAI Chat/Responses、Anthropic、Gemini、MCP Streamable HTTP；`TestMCPRoutesUseExactConfiguredUpstreamPath` 固定任意配置端点；`TestLegacySSEManagerBindsExactCapabilityAndOwnsVault` 固定 legacy SSE 双端点所需的有界状态与撤销边界 | MCP legacy SSE 已独立建模为 `mcp_legacy_sse` 并具有状态管理基础，但 endpoint 事件改写与 GET/POST 数据面尚未接通；完成有状态双端点适配前不能把整个 MCP 范围标为完成 |
